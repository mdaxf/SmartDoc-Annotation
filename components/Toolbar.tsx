...
import { ToolType, SmartDocMode } from '../types';
...
import { 
  ...
  ZoomIn, ZoomOut, AlertTriangle, ListChecks, Maximize, Minimize,
  MessageSquare, GalleryVertical, X
} from 'lucide-react';
...
const Toolbar: React.FC<ToolbarProps> = ({
  ...
  if (mode === 'edit') {
      visibleTools = allTools.filter(t => t.id === 'select' || t.id === 'hand');
  } else if (mode === 'viewonly') {
      visibleTools = allTools.filter(t => t.id === 'hand');
  }

  // Determine read-only state for UI elements
  const isViewOnly = mode === 'viewonly';
  const isEditMode = mode === 'edit';
  const canModify = !isViewOnly;

  if (variant === 'bottom') {
      return (
        <div 
          className="absolute bottom-6 left-1/2 transform -translate-x-1/2 flex items-center bg-gray-800/95 backdrop-blur border border-gray-600 rounded-2xl shadow-2xl p-2 gap-4 z-50 animate-in slide-in-from-bottom-5 overflow-x-auto"
        ...
        >
            ...
            <div className="flex gap-1">
                {visibleTools.map((tool) => (
                    <button
                        key={tool.id}
                        type="button"
                        onClick={(e) => { e.preventDefault(); setTool(tool.id as ToolType); }}
            ...
            {selectedAnnotationId && canModify && (
                ...
                <div className="flex items-center gap-1">
                    <button type="button" onClick={(e) => { e.preventDefault(); onEditSelected && onEditSelected(); }} className="p-2 hover:bg-gray-700 text-blue-400 rounded-xl" title="Edit Selected">
                        <MessageSquare className="w-5 h-5" />
                    </button>
                    <button type="button" onClick={(e) => { e.preventDefault(); onDeleteSelected && onDeleteSelected(); }} className="p-2 hover:bg-red-900/50 text-red-400 rounded-xl" title="Delete Selected">
            ...
            <div className="flex items-center gap-2">
                 ...
                 <button type="button" onClick={(e) => { e.preventDefault(); handleZoomOut(); }} className="p-2 hover:bg-gray-700 rounded-lg text-gray-300">
                    <ZoomOut className="w-4 h-4" />
                </button>
                <button type="button" onClick={(e) => { e.preventDefault(); handleZoomIn(); }} className="p-2 hover:bg-gray-700 rounded-lg text-gray-300">
                    <ZoomIn className="w-4 h-4" />
                </button>
                <button type="button" onClick={(e) => { e.preventDefault(); onToggleFullscreen(); }} className="p-2 hover:bg-gray-700 rounded-lg text-gray-300 border-l border-gray-600 ml-1" title={isFullscreen ? "Exit Fullscreen" : "Fullscreen"}>
                    {isFullscreen ? <Minimize className="w-4 h-4" /> : <Maximize className="w-4 h-4" />}
                </button>
            </div>

            {!isViewOnly && <div className="w-px h-8 bg-gray-600 flex-shrink-0" />}
             
             {/* Attributes - Only show in Edit/Full mode */}
            {!isViewOnly && (
                <div className="flex items-center gap-2">
                     <div className="flex gap-1">
  ...
  const showDocumentSection = (!hideLoadFileBtn) || (hasFile);
    ...
    >
      {isMobile && onClose && (
           <div className="flex justify-between items-center pb-2 border-b border-gray-700">
                ...
                <button type="button" onClick={(e) => { e.preventDefault(); onClose(); }} className="p-2 hover:bg-gray-700 rounded-full text-gray-400">
                    <X className="w-5 h-5" />
                </button>
           </div>
      )}
      ...
      {/* File Operations */}
      {showDocumentSection && !isViewOnly && (
          <div className="space-y-3">
            <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider">Document</h3>
            
            <div className="flex gap-2">
                {!hideLoadFileBtn && (
                    <label className="flex-1 flex items-center justify-center px-2 py-3 bg-indigo-600 hover:bg-indigo-700 rounded-lg cursor-pointer transition-colors text-xs font-bold text-white">
      ...
      <div className="space-y-3">
         ...
         <div className="flex items-center justify-between bg-gray-700 rounded-lg p-1">
            ...
            <button type="button" onClick={(e) => { e.preventDefault(); handleZoomOut(); }} className="p-2 hover:bg-gray-600 rounded text-gray-300" title="Zoom Out">
                <ZoomOut className="w-4 h-4" />
            </button>
            <span className="text-xs font-mono w-10 text-center">{(scale * 100).toFixed(0)}%</span>
            <button type="button" onClick={(e) => { e.preventDefault(); handleZoomIn(); }} className="p-2 hover:bg-gray-600 rounded text-gray-300" title="Zoom In">
                <ZoomIn className="w-4 h-4" />
            </button>
             <button type="button" onClick={(e) => { e.preventDefault(); onToggleFullscreen(); }} className="p-2 hover:bg-gray-600 rounded text-gray-300 border-l border-gray-600 ml-1" title={isFullscreen ? "Exit Fullscreen" : "Fullscreen"}>
      ...
      {!isViewOnly && <div className="h-px bg-gray-700" />}
      
      {/* Selected Annotation Actions */}
      {selectedAnnotationId && !isViewOnly && (
        <div className="space-y-3 p-3 bg-gray-700/50 rounded-lg border border-gray-600 animate-in zoom-in-95 duration-200">
            ...
            <div className="flex gap-2">
                <button 
                    type="button"
                    onClick={(e) => { e.preventDefault(); onEditSelected && onEditSelected(); }}
                ...
                <button 
                    type="button"
                    onClick={(e) => { e.preventDefault(); onDeleteSelected && onDeleteSelected(); }}
      ...
      {selectedAnnotationId && !isViewOnly && <div className="h-px bg-gray-700" />}

      {/* Defect Details - Hide in ViewOnly */}
      {!isViewOnly && (
        <div className="space-y-4">
                ...
                <div className="flex gap-2 flex-wrap">
                    {severityLevels.map(s => (
                        <button
                            key={s}
                            type="button"
                            onClick={(e) => { e.preventDefault(); setSeverity(s); }}
            ...
            {/* Width/Size - Hide in Edit mode if just editing attributes */}
            {mode === 'full' && (
                <div className="pt-2">
                    {currentTool === 'text' ? (
                        ...
                        <div className="flex gap-1 flex-wrap">
                            {FONT_SIZES.map(s => (
                                <button
                                    key={s}
                                    type="button"
                                    onClick={(e) => { e.preventDefault(); setFontSize(s); }}
                        ...
                        <div className="flex gap-1 flex-wrap">
                            {STROKE_WIDTHS.map(w => (
                                <button
                                    key={w}
                                    type="button"
                                    onClick={(e) => { e.preventDefault(); setStrokeWidth(w); }}
      ...
      <div className="space-y-3 mt-auto">
        ...
        {!hideSaveJsonBtn && (
            <button type="button" onClick={(e) => { e.preventDefault(); onSave(); }} className="flex items-center w-full px-3 py-2 text-sm text-gray-300 hover:text-white hover:bg-gray-700 rounded-lg transition-colors">
                <Download className="w-4 h-4 mr-3" />
                Save Annotations
            </button>
        )}
        
        {/* Load File Buttons - Hide in ViewOnly/Edit usually */}
        {!isViewOnly && !hideLoadJsonBtn && (
        ...
        {!isViewOnly && (
            <button type="button" onClick={(e) => { e.preventDefault(); onClear(); }} className="flex items-center w-full px-3 py-2 text-sm text-red-400 hover:text-red-300 hover:bg-red-900/20 rounded-lg transition-colors">
